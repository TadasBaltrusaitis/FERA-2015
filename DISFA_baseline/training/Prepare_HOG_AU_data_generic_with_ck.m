function [data_train, labels_train, data_valid, labels_valid, raw_valid, PC, means_norm, stds_norm] = ...
    Prepare_HOG_AU_data_generic_with_ck(train_users, au_train, rest_aus, hog_data_dir, ck_data_dir)
...
% Adding to the training data from the CK+ dataset
% First extract the labels and files of interest
[vid_inds, ck_labels, good_inds] = extract_ck_files(ck_data_dir, au_train);
ck_hog_data = Read_HOG_files_CK(vid_inds, 'C:\tadas\face_datasets\hog_aligned_rigid/');
good_inds = cat(2, good_inds, good_inds)';
good_inds = good_inds(:);
ck_labels = cat(2, zeros(size(ck_labels)), ck_labels)';
ck_labels = ck_labels(:);

ck_hog_data = ck_hog_data(good_inds,:);
ck_labels = ck_labels(good_inds,:);
...
ck_hog_data = bsxfun(@times, bsxfun(@plus, ck_hog_data, -means_norm), 1./stds_norm);

train_appearance_data = cat(1, train_appearance_data, ck_hog_data);
labels_train = cat(1, labels_train, ck_labels);
