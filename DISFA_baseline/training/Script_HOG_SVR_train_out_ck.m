function Script_HOG_SVR_train_out_ck()
...
for a=1:numel(aus)
    ...
    [train_samples, train_labels, valid_samples, valid_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_generic_with_ck(users(train_users), au, rest_aus, hog_data_dir, ck_dir);
    ...
    name = sprintf('trained_CK_static/AU_%d_dyn.dat', au);

    write_lin_dyn_svr(name, means, svs, b);

    name = sprintf('trained_CK_static/AU_%d_dyn.mat', au);
