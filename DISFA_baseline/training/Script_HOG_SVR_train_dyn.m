...
for a=1:numel(aus)
    ...
    [~, ~, test_samples, test_labels, ~, ~, ~, ~, valid_ids, success_test] = Prepare_HOG_AU_data_generic_dynamic(users(train_users), au, rest_aus, hog_data_dir);

    users_train = setdiff(users, unique(valid_ids));
    
    % need to split the rest
    [train_samples, train_labels, valid_samples, valid_labels, ~, PC, means, scaling, ~, success_valid] = Prepare_HOG_AU_data_generic_dynamic(users_train, au, rest_aus, hog_data_dir);
    ...
    best_params.success = success_test;
...
%     write_lin_dyn_svr(name, means, svs, b);
    ...
    name = sprintf('results_ideal/AU_%d_dyn.mat', au);
