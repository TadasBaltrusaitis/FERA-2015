function Script_HOG_SVR_train_dyn_out_ck()
...
shared_defs_dyn;
ck_dir = 'C:\tadas\face_datasets\ck+\';
...
for a=1:numel(aus)
    ...
    [train_samples, train_labels, valid_samples, valid_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_generic_dynamic_with_ck(users(train_users), au, rest_aus, hog_data_dir, ck_dir);
    ...
    name = sprintf('trained_CK_no_bad/AU_%d_dyn.dat', au);

    write_lin_dyn_svr(name, means, svs, b);

    name = sprintf('trained_CK_no_bad/AU_%d_dyn.mat', au);
