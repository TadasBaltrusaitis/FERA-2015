function Script_HOG_SVM_train_joint_dynamic_train_all()
...
for a=1:numel(aus)
    ...
    [train_samples_semaine, train_labels_semaine, valid_samples_semaine, valid_labels_semaine, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic_dynamic(train_recs, devel_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
    cd(od)
    ...
    train_samples = cat(1, train_samples_semaine, train_samples_bp4d, train_samples_disfa);
    train_labels = cat(1, train_labels_semaine, train_labels_bp4d, train_labels_disfa);
...
%     assert(norm(preds_mine - actual_vals) < 1e-8);
    ...
    name = sprintf('paper_res/AU_%d_dynamic_combined_all.dat', au);
    ...
    name = sprintf('paper_res/AU_%d_dynamic_combined_all.mat', au);
