...
for a=1:numel(all_aus)
    ...
    [train_samples, train_labels, valid_samples, valid_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_dynamic(train_recs, devel_recs, au, rest_aus, UNBC_dir, hog_data_dir, pca_loc);
    ...
    name = sprintf('results/AU_%d_dynamic_intensity.dat', au);
    write_lin_dyn_svr(name, means, svs, b);

    name = sprintf('results/AU_%d_dynamic_intensity.mat', au);
