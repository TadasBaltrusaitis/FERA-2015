    ...
    if(isdir([bp4d_loc, bp4d_dirs(f1).name]))
        ...
        for f2=1:numel(bp4d_2_dirs)
            ...
            if(isdir([bp4d_loc, bp4d_dirs(f1).name]))
                ...
                command = cat(2, command, [' -fx 2000 -fy 2000 -rigid -asvid -fdir "' curr_vid '" -simalign "' output_file  '" -simscale 0.6 -simsize 96 -g']);
                ...
                command = cat(2, command, [' -oneutral "' output_neut '"']);
                dos(command);
            end
        end
    end
