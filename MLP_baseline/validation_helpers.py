...
def validate_grid_search_cheat(train_fn, test_fn, minimise, samples_train, labels_train, samples_valid, labels_valid, hyperparams, num_repeat=1):
    #crossvalidate_regressor_grid_search A utility function for crossvalidating a statistical model
    #Detailed explanation goes here
    ...
    # Crossvalidate the c, p, and gamma values
    for p in range(num_params):
        all_params[p]["result"] = single_pass_cheat(train_fn, test_fn, labels_train, samples_train, labels_valid,
                                              samples_valid, all_params[p], num_repeat)
        print all_params[p]
...
def single_pass_cheat(train_fn, test_fn, labels_train, samples_train, labels_valid, samples_valid, hyperparams, num_repeat):
    result = 0

    for r in range(num_repeat):
        model = train_fn(labels_train, samples_train, labels_valid, samples_valid, hyperparams)
        result += test_fn(labels_valid, samples_valid, model)[0]
    result = result / num_repeat
    return result