function Script_HOG_SVM_train_shift()
...
svm_train = @svm_train_linear_shift;
    
% Set the test function (the first output will be used for validation)
svm_test = @svm_test_linear_shift;
...
for a=1:numel(aus)
    ...
    [~, ~, test_samples, test_labels, raw_test, ~,~,~, test_ids] = Prepare_HOG_AU_data_generic(train_recs, test_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
    
    [train_samples, train_labels, valid_samples, valid_labels, ~, PC, means, scaling, valid_ids] = Prepare_HOG_AU_data_generic(train_recs, valid_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
    ...
    [result, prediction] = svm_test(test_labels, test_samples, model);
...
%     write_lin_svm(name, means, svs, b, pos_lbl, neg_lbl);
    ...
    name = sprintf('results_ideal_shift/AU_%d_static.mat', au);
