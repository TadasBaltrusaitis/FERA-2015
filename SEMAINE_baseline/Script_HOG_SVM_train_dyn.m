...
for a=1:numel(aus)
    ...
    [~, ~, test_samples, test_labels, raw_test] = Prepare_HOG_AU_data_generic_dynamic(train_recs, test_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);    
    [train_samples, train_labels, valid_samples, valid_labels, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic_dynamic(train_recs, valid_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
...
%     write_lin_svm(name, means, svs, b, pos_lbl, neg_lbl);
    ...
    name = sprintf('results_ideal/AU_%d_dynamic.mat', au);
