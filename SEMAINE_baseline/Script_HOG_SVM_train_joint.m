function Script_HOG_SVM_train_joint()
...
for a=1:numel(aus)
    ...
    if(ismember(aus(a), bp4d_au) || ismember(aus(a), disfa_au))
        ...
        if(ismember(aus(a), bp4d_au))
            od = cd('../BP4D_baseline/');
            find_BP4D;
            rest_aus = setdiff(bp4d_au, au);    
            [train_samples_bp4d, train_labels_bp4d, ~, ~, ~, ~, ~, ~] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, rest_aus, BP4D_dir, hog_data_dir, pca_loc);
            cd(od);
