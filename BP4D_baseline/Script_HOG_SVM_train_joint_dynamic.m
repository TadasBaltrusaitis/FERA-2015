...
for a=1:numel(aus)
    ...
    if(ismember(aus(a), semaine_au) || ismember(aus(a), disfa_au))
        ...
        if(ismember(aus(a), semaine_au))
            ...
            [train_samples_sem, train_labels_sem, ~, ~, ~, ~, ~, ~] = Prepare_HOG_AU_data_generic_dynamic(train_recs, devel_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
            cd(od);
        ...
        name = sprintf('camera_ready/AU_%d_dynamic_joint.dat', au);

        write_lin_dyn_svm(name, means, svs, b, model.Label(1), model.Label(2));

        name = sprintf('camera_ready/AU_%d_dynamic_joint.mat', au);
