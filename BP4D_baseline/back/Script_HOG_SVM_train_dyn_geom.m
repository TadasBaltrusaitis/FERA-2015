...
for a=1:numel(aus)
    ...
    [train_samples, train_labels, valid_samples, valid_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_generic_geom_dynamic(train_recs, devel_recs, au, BP4D_dir, hog_data_dir, pca_loc);
...
%     write_lin_svm(name, means, svs, b, model.Label(1), model.Label(2));
    ...
    name = sprintf('results_geom/AU_%d_dynamic_loc.mat', au);
