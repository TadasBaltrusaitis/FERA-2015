function Script_HOG_SVR_train_joint_v_all()
...
for a=1:numel(all_aus_int)
    ...
    [train_samples, train_labels, vid_ids_train, valid_samples, valid_labels, vid_ids_valid, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic_intensity(train_recs, devel_recs, au, BP4D_dir_int, hog_data_dir, pca_loc);

    find_DISFA;
    od = cd('../DISFA_baseline/training/');
    all_disfa = [1,2,4,5,6,9,12,15,17,20,25,26];
    rest_aus = setdiff(all_disfa, au);    
    [train_samples_disfa, train_labels_disfa, valid_disfa, valid_labels_disfa, ~, ~, ~, ~] = Prepare_HOG_AU_data_generic(users, au, rest_aus, hog_data_dir);            
    cd(od);

    train_samples = cat(1, train_samples, train_samples_disfa);
    train_labels = cat(1, train_labels, train_labels_disfa);    
    
    valid_samples = cat(1, valid_samples, valid_disfa);
...
%     assert(norm(preds_mine - actual_vals) < 1e-8);
    ...
    name = sprintf('new_tracker/AU_%d_static_intensity_combined_v_all.dat', au);
    write_lin_svr(name, means, svs, b);

    name = sprintf('new_tracker/AU_%d_static_intensity_combined_v_all.mat', au);
