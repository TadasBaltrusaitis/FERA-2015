function Script_HOG_SVM_train_disfa_static()
...
for a=1:numel(aus)
    ...
    if(ismember(aus(a), disfa_au))
        ...
        [~, ~, valid_samples, valid_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, BP4D_dir, hog_data_dir, pca_loc);
        ...
        name = sprintf('camera_ready/AU_%d_static_DISFA.dat', au);

        write_lin_svm(name, means, svs, b, model.Label(1), model.Label(2));

        name = sprintf('camera_ready/AU_%d_static_DISFA.mat', au);
