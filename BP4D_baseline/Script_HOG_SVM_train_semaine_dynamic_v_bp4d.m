function Script_HOG_SVM_train_semaine_dynamic_v_bp4d()
...
for a=1:numel(aus)
    ...
    if(ismember(aus(a), semaine_au))
        ...
        [~, ~, test_samples, test_labels, raw_valid, PC, means, scaling] = Prepare_HOG_AU_data_generic_dynamic(train_recs, devel_recs, au, BP4D_dir, hog_data_dir, pca_loc);
        ...
        [train_samples_semaine, train_labels_semaine, ~, ~, ~, ~, ~, ~] = Prepare_HOG_AU_data_generic_dynamic(train_recs, devel_recs, au, rest_aus, SEMAINE_dir, hog_data_dir, pca_loc);
        cd(od);
        ...
        name = sprintf('camera_ready/AU_%d_dynamic_SEMAINE_v_bp4d.dat', au);
        ...
        name = sprintf('camera_ready/AU_%d_dynamic_SEMAINE_v_bp4d.mat', au);
