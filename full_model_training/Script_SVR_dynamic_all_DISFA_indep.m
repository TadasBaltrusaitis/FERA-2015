...
for a=1:numel(aus)
    ...
    if(use_semaine)
        ...
        name = sprintf('../SEMAINE_baseline/results_regression/AU_%d_dynamic_shift.mat', au_current);
        load(name, 'model');
        model_sem = model;
    ...
    if(use_bp4d_class)
        % These have been trained already so just load them
        name = sprintf('../BP4D_baseline/results_regression/AU_%d_dynamic.mat', au_current);
        load(name, 'model');
        model_bp4d = model;
...
%     p = predictions \ valid_labels_d;
    ...
    name = sprintf('independently/AU_%d_dynamic.mat', au_current);
