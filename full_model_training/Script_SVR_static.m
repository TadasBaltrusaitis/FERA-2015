function Script_HOG_SVM_train_joint_static_train_all()
...
for a=1:numel(aus)
    ...
    if(use_semaine)
        ...
        [train_samples_semaine, train_labels_semaine, valid_samples_semaine, valid_labels_semaine, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic(train_recs, devel_recs, au, rest_aus_sem, SEMAINE_dir, hog_data_dir, pca_loc);
        cd(od)
    ...
    if(use_disfa)
        ...
        [train_samples_disfa, train_labels_disfa, valid_samples_disfa, valid_labels_disfa, ~, PC, means, scaling] = Prepare_HOG_AU_data_generic(users, au, rest_aus, hog_data_dir);            
        % Binarise the models            
...
%     assert(norm(preds_mine - actual_vals) < 1e-8);
    ...
    name = sprintf('new_tracker/AU_%d_static_combined_all.dat', au);
    ...
    name = sprintf('new_tracker/AU_%d_static_combined_all.mat', au);
